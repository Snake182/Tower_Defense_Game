...
bool Level::createDecor(int level_number) {
	...
	vector<tuple<float, float, float, char*>> modelsPositions;
	...
	for (pugi::xml_node xml_model = xml_models.first_child(); xml_model; xml_model = xml_model.next_sibling())
	{
		...
		modelsPositions.push_back ( make_tuple ( pozHorizontal, pozVertical, size, c_textureFile) );
	}
...
vector<Model*> Level::createModels(vector<tuple<float, float, float, char*>> modelsPositions)
{
	vector<Model*> models;
	for (int i = 0; i < modelsPositions.size(); i++) {
		models.push_back(new Model(get<0>(modelsPositions.at(i)), get<1>(modelsPositions.at(i)), get<2>(modelsPositions.at(i)), get<3>(modelsPositions.at(i))));
	}
	return models;
}